...
" in
  ...
  print_endline "\n[4] Epiplexity & Structural Information Extraction (epiplexity):";
  let epi_code = "
let bio_stream = [12.0, 14.5, 18.2, 22.0, 27.1, 33.0, 39.8, 48.0];
let profile = bio_stream -> epiplexity(budget: 150);
" in
  let e_prog = Parser.program_file Lexer.read (Lexing.from_string epi_code) in
  let e_results = Eval.eval_program e_prog in
  List.iter (fun s -> print_endline ("  " ^ s)) e_results;
