...
\lstdefinelanguage{neu}{
  keywords={fn, let, in, if, then, else, true, false, type, contract, rule, learn, split, scatter, decompose, cover, shift, cast, synthesis, analysis, transformation},
...
We have established the theoretical foundations and syntactic blueprint for \texttt{learn} in Neu:
\begin{itemize}
...
In forthcoming work, we will implement the Menhir AST grammar extensions for \texttt{learn}, integrate an automatic differentiation runtime in OCaml, and benchmark Neu's compiled topological models directly against PyTorch on standard biosignal and linguistic tasks.
