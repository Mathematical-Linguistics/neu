...
let en_ja = en_input -> cover(w=2, s=1) -> shift(1) -> cast("Japanese");
    ...
    English declarative continuous phrases split into high-tone progressive aspect particles (\emph{"ń"}) and root verbs (\emph{"jẹ"} / \emph{"mu"}), reversing noun-determiner ordering (\emph{"The elder"} $\to$ \emph{"Àgbàlagbà náà"}):
    \begin{lstlisting}[language=neu]
...
\subsection{Two-Tier Cascade Architecture}
The Whisplay backend integrates \texttt{neu\_bridge} into its standard HTTP proxy pipeline. When a translation request arrives:
\begin{enumerate}
    \item The backend normalizes the input tokens and evaluates the Neu topological manifold.
    \item If a categorical morphism exists, Neu returns the exact translated token string in $< 3\,\mu\text{s}$.
    \item If the client requested SSE streaming (\texttt{stream: true}), the backend immediately synthesizes an OpenAI-compliant chunked response (\verb|data: {"choices": [...]}|), terminating with \verb|data: [DONE]|.
    \item The client-side sentence buffer instantly captures the complete translated clause and immediately queues TTS synthesis via Moonshine-Voice, bypassing the 1.2-second LLM wait.
    \item If the query falls outside the pre-compiled manifold, the request falls through unimpeded to LiteRT-LM hosting Gemma 4.
\end{enumerate}
